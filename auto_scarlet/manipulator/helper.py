...
class HelperFunctions:
    ...
    def similarity_index(state_path, comp_path):
        state_np = np.array(Image.open(state_path))
        comp_np = np.array(Image.open(comp_path))
        similarity_index, _ = ssim(state_np, comp_np,
                                   win_size=min(min(state_np.shape[0],
                                                    state_np.shape[1],
                                                    comp_np.shape[0],
                                                    comp_np.shape[1]), 3),
                                   full=True)
    ...
    def riddle_solver(riddle):
        question = {
            ...
            "Which of the following attributes can the wings [Light of Holy Dome] provide?": "B",
            "How long does the [Pinnacle Arena] take each season?": "B: 14 days"
        }
        if riddle in question:
